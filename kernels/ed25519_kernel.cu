...
/* ================= Task 8: group arithmetic + scalarmult_base ================= */

__device__ static const bignum25519 ge25519_ecd = {
    0x00034dca135978a3,0x0001a8283b156ebd,0x0005e7a26001c029,0x000739c663a03cbb,0x00052036cee2b6ff
};
__device__ static const bignum25519 ge25519_ec2d = {
    0x00069b9426b2f159,0x00035050762add7a,0x0003cf44c0038052,0x0006738cc7407977,0x0002406d9dc56dff
};

typedef struct ge25519_t      { bignum25519 x, y, z, t; } ge25519;
typedef struct ge25519_p1p1_t { bignum25519 x, y, z, t; } ge25519_p1p1;
typedef struct ge25519_niels_t { bignum25519 ysubx, xaddy, t2d; } ge25519_niels;

/* ---- scalar arithmetic mod the group order (modm) ---- */
typedef uint64_t bignum256modm_element_t;
typedef bignum256modm_element_t bignum256modm[5];

__device__ static const bignum256modm modm_m = {
    0x12631a5cf5d3ed, 0xf9dea2f79cd658, 0x000000000014de, 0x00000000000000, 0x00000010000000
};
__device__ static const bignum256modm modm_mu = {
    0x9ce5a30a2c131b, 0x215d086329a7ed, 0xffffffffeb2106, 0xffffffffffffff, 0x00000fffffffff
};

__device__ __forceinline__ static uint64_t
U8TO64_LE_dev(const unsigned char *p) {
...
__device__ __forceinline__ static bignum256modm_element_t
...
expand256_modm(bignum256modm out, const unsigned char *in, uint32_t len) {
    ...
    for (i = 0; i < 64; i++) work[i] = 0;
    for (i = 0; i < len; i++) work[i] = in[i];

    x[0] = U8TO64_LE_dev(work +  0);
    x[1] = U8TO64_LE_dev(work +  8);
    x[2] = U8TO64_LE_dev(work + 16);
    x[3] = U8TO64_LE_dev(work + 24);
    x[4] = U8TO64_LE_dev(work + 32);
    x[5] = U8TO64_LE_dev(work + 40);
    x[6] = U8TO64_LE_dev(work + 48);
    x[7] = U8TO64_LE_dev(work + 56);
...
/* ---- conditional helpers (bytewise / limbwise; alignment-safe under NVRTC) ---- */
__device__ __forceinline__ static void
curve25519_move_conditional_bytes(uint8_t out[96], const uint8_t in[96], uint64_t flag) {
    const uint8_t nb = (uint8_t)(flag - 1);   /* 0xff if flag==0, 0x00 if flag==1 */
    const uint8_t b  = (uint8_t)(~nb);
    for (int k = 0; k < 96; k++)
        out[k] = (out[k] & nb) | (in[k] & b);
}
...
/* ---- point conversions / doubling / mixed addition ---- */
__device__ __forceinline__ static void
...
/* ---- pack (point compression to 32 bytes) ---- */
__device__ static void
...
/* ---- precomputed base-point multiples, packed {ysubx, xaddy, t2d} ---- */
__constant__ uint8_t ge25519_niels_base_multiples[256][96] = {
...
/* triage helper: [0]B should pack to the identity 0x01,0x00,...,0x00 */
extern "C" __global__
void scalarmult_zero_test(unsigned char *out) {
    bignum256modm a;
    ge25519 A;
